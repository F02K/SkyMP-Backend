...
const DEFAULT_BRANCH = config.integrations.clientBranch
let updateRunning = false
let currentJob = null

function sendError(req, res, status, code, message) {
  if (req.baseUrl.startsWith('/api/v2/')) return res.status(status).json({ error: { code, message } })
...
function pullAndMerge() {
  if (updateRunning) {
    console.warn('[webhook] client update already running; duplicate delivery skipped')
    return currentJob
  }
  updateRunning = true
  console.log('[webhook] Running git pull in', CLIENT_DIR)

  currentJob = new Promise((resolve, reject) => {
    execFile('git', ['-C', CLIENT_DIR, 'pull', '--ff-only'], (error, stdout, stderr) => {
    ...
    .then(summary => {
      ...
      return mergeSourcesIntoRoot()
    })
    .catch(error => console.error('[webhook] client publish failed:', error.message))
    .finally(() => {
      updateRunning = false
      currentJob = null
    })
...
router.post('/github', githubWebhookLimiter, (req, res) => {
  const secret = config.secrets.githubWebhookSecret

  // Secret must be configured — reject silently otherwise
  if (!secret) {
    console.error('[webhook] GITHUB_WEBHOOK_SECRET is not set.')
    return sendError(req, res, 500, 'webhookNotConfigured', 'Webhook is not configured on this server.')
  }
  ...
  if (!verifySignature(secret, req.rawBody, sig)) {
    ...
    return sendError(req, res, 401, 'invalidWebhookSignature', 'Invalid webhook signature.')
  }
...
router.waitForIdle = () => currentJob || Promise.resolve()
router.pullAndMerge = pullAndMerge
